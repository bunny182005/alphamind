...
# Icons + colors shared by the progress table, message log, and final report.
STATUS_ICON = {
    "pending": "○",
    "in_progress": "◔",
    "completed": "●",
...
def render_progress_bar(completed: int, total: int, width: int = 24) -> Text:
    """Render a compact block-style progress bar with a red→yellow→green gradient."""
    pct = 0.0 if total == 0 else completed / total
    ...
    bar.append(f" {pct*100:3.0f}%", style=f"bold {color}")
    return bar


def update_display(layout, spinner_text=None, stats_handler=None, start_time=None):
    # Header with welcome banner + a live elapsed-time readout
    header_text = Text()
    header_text.append("◆ ", style="bold green3")
    header_text.append("AlphaMind AI", style="bold green3")
    header_text.append("  —  Multi-Agent LLM Trading Analysis", style="dim italic")
    if start_time:
        elapsed = time.time() - start_time
        header_text.append(
            f"   ⏱ {int(elapsed // 60):02d}:{int(elapsed % 60):02d}", style="bold cyan"
        )
    ...
    progress_table = Table(
        ...
        show_footer=False,
        box=box.ROUNDED,  # Softer rounded borders read better than plain rules
...
def get_user_selections():
   ...
   # Create welcome box content
    ...
    welcome_content += "[bold green]AlphaMind AI: Multi-Agents LLM Financial Trading Framework - CLI[/bold green]\n\n"
